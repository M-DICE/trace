...
def run(quick: bool, plots_only: bool) -> None:
    ...
    if not plots_only:
        ...
        detection_results_iid = run_simulation_iid(
            simN=simN, trend_increase=trend_increase, crit_val=crit_val,
            npre=npre, ntt=ntt, npost_max=npost_max,
            level=level, trend_control=trend_control, sigma=sigma, delay_set=delay_set,
            existing_results=detection_results_iid, on_trend_done=on_done_iid,
        )
        _save()
    ...
    _generate_plots(plots_dir, detection_results_iid, detection_results_ar,
                    trend_increase, npre, npost_max, level, trend_control, sigma)
...
def _generate_plots(plots_dir, detection_results_iid, detection_results_ar,
                    trend_increase, npre, npost_max, level, trend_control, sigma):
    print("Generating plots...")
    example_trend = trend_increase[3]
    res = detection_results_iid[example_trend]
    ex_sim_data = ci_sim(
        seed=res['seeds'][0], npre=npre + res['delays'][0],
    ...
    plot_time_series(ex_sim_data, npre, stats=ex_stats,
