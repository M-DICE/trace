...
def run(quick: bool, plots_only: bool) -> None:
    ...
    if not plots_only:
        ...
        detection_results_baci = run_simulation_cdf_baci(
            simN=simN, trend_increase_mu=trend_increase_mu, crit_val=crit_val,
            npre=npre, ntt=ntt, npost_max=npost_max,
            mu=mu, sigma=sigma, ns=ns, delay_set=delay_set,
            existing_results=detection_results_baci, on_trend_done=on_done_baci,
        )
        _save()
        ...
        detection_results_ba = run_simulation_cdf_ba(
            ...
            npre=npre, ntt=ntt, npost_max=npost_max,
            mu=mu, sigma=sigma, ns=ns, delay_set=delay_set,
            existing_results=detection_results_ba, on_trend_done=on_done_ba,
        )
        _save()
    ...
    _generate_plots(plots_dir, detection_results_baci, detection_results_ba,
                    trend_increase_mu, npost_vec, npre)
...
def _generate_plots(plots_dir, detection_results_baci, detection_results_ba,
                    trend_increase_mu, npost_vec, npre):
    ...
    plot_power_curves(detection_results_baci, trend_increase_mu, npost_vec,
                      savefile=f"{plots_dir}/power_curves_baci.png")
    plot_power_curves(detection_results_ba, trend_increase_mu, npost_vec,
                      savefile=f"{plots_dir}/power_curves_ba.png")
    plot_detection_heatmap(detection_results_baci, None, trend_increase_mu, npost_vec,
                           savefile=f"{plots_dir}/detection_heatmap_baci.png")
    plot_detection_heatmap(detection_results_ba, None, trend_increase_mu, npost_vec,
                           savefile=f"{plots_dir}/detection_heatmap_ba.png")

    plot_forecast_detection_summary(
        detection_results_baci, trend_increase_mu, npre,
        title="Distribution Forecast BACI",
        savefile=f"{plots_dir}/summary_table_baci.csv",
    )
    plot_forecast_detection_summary(
        detection_results_ba, trend_increase_mu, npre,
