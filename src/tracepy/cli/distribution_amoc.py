...
def run(quick: bool, plots_only: bool) -> None:
    ...
    if not plots_only:
        ...
        print("=" * 70)
        print(f"PHASE 2A: Mean Change BACI  ({len(detection_results_mu)}/{len(trend_increase_mu)} cached)")
        print("=" * 70)
        t0 = time.perf_counter()
        ...
        detection_results_mu = run_main_simulation_mu(
            ...
            existing_results=detection_results_mu, on_trend_done=on_done_mu,
        )
        _save()
        print(f"  Phase 2A total: {fmt_elapsed(time.perf_counter() - t0)}")

        print("=" * 70)
        print(f"PHASE 2A': Mean Change BA  ({len(detection_results_mu_ba)}/{len(trend_increase_mu)} cached)")
        print("=" * 70)
        t0 = time.perf_counter()
        ...
        detection_results_mu_ba = run_main_simulation_mu(
            ...
            existing_results=detection_results_mu_ba, on_trend_done=on_done_mu_ba,
        )
        _save()
        print(f"  Phase 2A' total: {fmt_elapsed(time.perf_counter() - t0)}")

        print("=" * 70)
        print(f"PHASE 2B: Variance Change  ({len(detection_results_sigma)}/{len(trend_increase_sigma)} cached)")
        print("=" * 70)
        t0 = time.perf_counter()
        ...
        detection_results_sigma = run_main_simulation_sigma(
            ...
            mu, sigma, ns, dist_measure, bw, nd, delay_set,
            existing_results=detection_results_sigma, on_trend_done=on_done_sigma,
        )
        _save()
    ...
    _print_summary(detection_results_mu, detection_results_sigma,
                   trend_increase_mu, trend_increase_sigma)
    _generate_plots(plots_dir, critical_values, detection_results_mu, detection_results_mu_ba,
                    detection_results_sigma, trend_increase_mu, trend_increase_sigma,
                    npost_vec, npre, npost_max, mu, sigma, ns, dist_measure, bw, nd)


def _print_summary(detection_results_mu, detection_results_sigma,
                   trend_increase_mu, trend_increase_sigma):
...
def _generate_plots(plots_dir, critical_values, detection_results_mu, detection_results_mu_ba,
                    detection_results_sigma, trend_increase_mu, trend_increase_sigma,
                    npost_vec, npre, npost_max, mu, sigma, ns, dist_measure, bw, nd):
    print("\nGenerating plots...")
    example_mu = trend_increase_mu[5]
    res = detection_results_mu[example_mu]
    sim_data = ci_sim_cdf(seed=res['seeds'][0], npre=npre + res['delays'][0],
    ...
    plot_distance_time_series(sim_data, npre, dist_ts, stats=ex_stats,
                              dist_measure=dist_measure,
                              savefile=f"{plots_dir}/example_distance_time_series.png")
    plot_distribution_difference(sim_data, npre, dist_ts, stats=ex_stats,
                                  dist_measure=dist_measure,
    ...
    plot_simulation_results(
        ...
        trend_increase_mu, savefile=f"{plots_dir}/sensitivity_analysis_mu.png",
    )
    plot_simulation_results(
        [detection_results_sigma[t]['detection_rates'][-1] for t in trend_increase_sigma],
        trend_increase_sigma, savefile=f"{plots_dir}/sensitivity_analysis_sigma.png",
    )
