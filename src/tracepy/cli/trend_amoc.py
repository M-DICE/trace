...
def run(quick: bool, plots_only: bool) -> None:
    ...
    def _save():
        save_simulation_results(FOLDER, {
    ...
    if not plots_only:
        ...
        detection_results = run_main_simulation(
            ...
            existing_results=detection_results, on_trend_done=on_done_iid,
        )
        _save()
        ...
        detection_results_ar = run_main_simulation_ar(
            ...
            delay_set=delay_set, existing_results=detection_results_ar,
        ...
        detection_results_iid_ba = run_main_simulation_iid_ba(
            ...
            npre=npre, npost_max=npost_max, npost_vec=npost_vec,
            level=level, trend_control=trend_control, sigma=sigma, delay_set=delay_set,
            existing_results=detection_results_iid_ba, on_trend_done=on_done_ba,
        )
        _save()
    ...
    _print_summary(critical_values, detection_results, detection_results_ar,
                   detection_results_iid_ba, trend_increase, npost_max)
    _generate_plots(plots_dir, critical_values, detection_results, detection_results_ar,
                    detection_results_iid_ba, sim_data_test, stats_test,
                    trend_increase, npost_vec, npre)


def _print_summary(critical_values, detection_results, detection_results_ar,
                   detection_results_iid_ba, trend_increase, npost_max):
    ...
    print(f"  i.i.d. BACI cv: {critical_values[f'iid_{CRITICAL_VALUE_NPOST_SHORT}']['critical_value']:.4f}")
    print(f"  AR(1)  BA   cv: {critical_values[f'ar1_{CRITICAL_VALUE_NPOST_SHORT}']['critical_value']:.4f}")
    if has_ba:
        print(f"  i.i.d. BA   cv: {critical_values[f'iid_ba_{CRITICAL_VALUE_NPOST_SHORT}']['critical_value']:.4f}")
    print("=" * len(col))
    print(col)
    print(sep)
    for trend_inc in trend_increase:
        ...
        line = (f"{trend_inc:>8.4f}  {iid['detection_rates'][-1]:>11.2%}  {iid['mean_errors'][-1]:>9.2f}m  "
...
def _generate_plots(plots_dir, critical_values, detection_results, detection_results_ar,
                    detection_results_iid_ba, sim_data_test, stats_test,
                    trend_increase, npost_vec, npre):
    print("Generating plots...")
    plot_simulation_results(
        [detection_results[t]['detection_rates'][-1] for t in trend_increase],
        trend_increase, savefile=f"{plots_dir}/sensitivity_analysis.png",
    )
    plot_time_series(sim_data_test, npre, stats=stats_test,
                     savefile=f"{plots_dir}/example_time_series.png")
    plot_power_curves(detection_results, trend_increase, npost_vec,
                      savefile=f"{plots_dir}/power_curves_iid.png")
    plot_power_curves(detection_results_ar, trend_increase, npost_vec,
                      savefile=f"{plots_dir}/power_curves_ar1.png")
    plot_power_curves_comparison(detection_results, detection_results_ar, trend_increase, npost_vec,
                                  savefile=f"{plots_dir}/power_curves_comparison.png")
    plot_time_to_detection(detection_results, detection_results_ar, trend_increase, npost_vec,
                           savefile=f"{plots_dir}/time_to_detection.png")
    plot_detection_heatmap(detection_results, detection_results_ar, trend_increase, npost_vec,
                           savefile=f"{plots_dir}/detection_heatmap.png")
    plot_mean_error_curves(detection_results, detection_results_ar, trend_increase, npost_vec,
                           savefile=f"{plots_dir}/mean_error_curves.png")
    plot_null_distributions(critical_values, npost_short=CRITICAL_VALUE_NPOST_SHORT,
                            npost_long=CRITICAL_VALUE_NPOST_LONG,
                            savefile=f"{plots_dir}/null_distributions.png")
    plot_tmax_signal_vs_noise(critical_values, detection_results, detection_results_ar,
                               trend_increase, npost_vec, npost_short=CRITICAL_VALUE_NPOST_SHORT,
                               savefile=f"{plots_dir}/tmax_signal_vs_noise.png")
    plot_detection_by_delay(detection_results, detection_results_ar, trend_increase,
