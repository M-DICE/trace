...
def plot_time_series(sim_data, npre, stats=None, figsize=(12, 6), savefile=None):
    ...
    axes[0].plot(
        t, sim_data["y_itv"], label="Intervention", color=CLR_ITV, alpha=0.7, linewidth=1.5
    )
    ...
    axes[0].set_title(
        "Simulated Time Series: Control vs Intervention", fontsize=12, fontweight="bold"
    )
    ...
    axes[1].plot(
        t,
        y_dif,
        label="Difference (Intervention − Control)",
        color=CLR_DIF,
        alpha=0.8,
        linewidth=1.5,
    )
...
def plot_test_statistics(
    test_stats_vec, npre, npost, detected_cpt=None, figsize=(10, 5), savefile=None
):
    """
    ...
    ax.set_title(
        "AMOC Test Statistics Across Possible Changepoint Locations", fontsize=12, fontweight="bold"
    )
    ax.legend(loc="best")
...
def plot_simulation_results(detection_rates, trend_increments, figsize=(10, 6), savefile=None):
    """
    ...
    ax.set_title(
        "Sensitivity Analysis: Detection Rate vs Trend Increment", fontsize=12, fontweight="bold"
    )
    ax.set_ylim([0, 1.05])
...
def plot_detection_error_distribution(detection_errors, figsize=(10, 5), savefile=None):
    ...
    ax.axvline(
        x=0, color=CLR_TAU_TRUE, linestyle="--", linewidth=2, label="Zero Error (Perfect Detection)"
    )
...
def plot_power_curves(detection_results, trend_increase, npost_vec, figsize=(12, 7), savefile=None):
    """
    ...
    for color, trend_inc in zip(colors, trend_increase):
        ...
        ax.plot(npost_vec, rates, color=color, linewidth=1.8, label=f"trend = {trend_inc:.4f}")

    ax.set_xlabel("Post-intervention length (months)", fontsize=11)
    ax.set_ylabel("Detection rate", fontsize=11)
    ax.set_title(
        "Power Curves: Detection Rate vs Post-Intervention Length\n(i.i.d. noise, growing window)",
        fontsize=12,
        fontweight="bold",
    )
    ax.set_ylim([0, 1.05])
...
def plot_power_curves_comparison(
    ...
    for ax, results, title in [
        ...
        for color, trend_inc in zip(colors, trend_increase):
            ...
            ax.plot(npost_vec, rates, color=color, linewidth=1.8, label=f"{trend_inc:.4f}")
    ...
    fig.suptitle(
        "Power Curves: i.i.d. vs AR(1) Noise\n"
...
def plot_time_to_detection(
    ...
    ax.set_title(
        "Time-to-Detection: Months of Monitoring Needed\nto Reach Target Power",
        fontsize=12,
        fontweight="bold",
    )
...
def plot_detection_heatmap(
    ...
    for ax, results, title in [
        ...
        if results is None:
            ax.text(
                0.5, 0.5, "Results not available", ha="center", va="center", transform=ax.transAxes
            )
            ax.set_title(title, fontsize=12, fontweight="bold")
            continue
        ...
        im = ax.imshow(
            ...
            aspect="auto",
            origin="lower",
            extent=[npost_vec[0], npost_vec[-1], -0.5, len(trend_increase) - 0.5],
            vmin=0,
            vmax=1,
            cmap="viridis",
        )
        ...
        labels = ax.clabel(
            cs, fmt={0.80: "80%", 0.95: "95%"}, fontsize=9, inline=True, inline_spacing=8
        )
        for lbl in labels:
    ...
    fig.suptitle(
        "Detection Rate Heatmap: Trend × Post-Intervention Length\n"
...
def plot_mean_error_curves(
    ...
    for ax, results, title in [
        ...
        for color, trend_inc in zip(colors, trend_increase):
            ...
            ax.plot(npost_vec, errors, color=color, linewidth=1.8, label=f"{trend_inc:.4f}")
        ax.set_xlabel("Post-intervention length (months)", fontsize=11)
        ax.set_title(title, fontsize=12, fontweight="bold")
        ax.grid(True, alpha=0.3)
    ...
    fig.suptitle(
        "Changepoint Localisation Error vs Post-Intervention Length", fontsize=13, fontweight="bold"
    )
    plt.tight_layout(rect=[0, 0, 0.88, 1])
    if savefile:
        plt.savefig(savefile, dpi=150, bbox_inches="tight")
...
def plot_null_distributions(
    critical_values, npost_short=48, npost_long=96, figsize=(14, 6), savefile=None
):
    """
    ...
    if all(k in critical_values for k in standard_keys):
        keys = standard_keys
        titles = [
            f"i.i.d. noise, npost = {s} mo",
            f"i.i.d. noise, npost = {l} mo",
            f"AR(1) noise,  npost = {s} mo",
            f"AR(1) noise,  npost = {l} mo",
        ]
    ...
    for ax, key, title, color in zip(axes, keys, titles, colors):
        ...
        ax.axvline(cv, color=CLR_TAU_DET, linewidth=2, label=f"CV = {cv:.3f}\n(95th pct)")
    ...
    fig.suptitle(
        "Null Distribution of Tmax (no trend change)\nCrimson line = critical value at α = 95%",
        fontsize=13,
        fontweight="bold",
    )
    plt.tight_layout()
...
def plot_tmax_signal_vs_noise(
    ...
    for ax, trend_inc in zip(axes, picked_trends):
        ...
        ax.axvline(
            cv_iid, color=CLR_BACI, linewidth=1.5, linestyle="--", label=f"CV i.i.d. = {cv_iid:.2f}"
        )
        ax.axvline(
            cv_ar, color=CLR_AR1, linewidth=1.5, linestyle=":", label=f"CV AR(1) = {cv_ar:.2f}"
        )
    ...
    axes[0].set_ylabel("Count", fontsize=11)
    fig.suptitle(
        "Tmax Distribution: Null vs Alternative\n"
...
def plot_detection_by_delay(
    detection_results_iid, detection_results_ar, trend_increase, figsize=(12, 6), savefile=None
):
    """
    ...
    for ax, trend_inc in zip(axes, picked_trends):
        for results, label, color, marker in [
            ...
            rates = [
                detected[delays == d].mean() if (delays == d).any() else np.nan for d in delay_vals
            ]
    ...
    fig.suptitle(
        "Effect of Intervention Delay on Detection Rate\n"
...
def plot_changepoint_bias(
    ...
    for ax, results, title, color in [
        ...
        ax.set_xticklabels(
            [f"{t:.4f}" for t in trend_increase], rotation=45, ha="right", fontsize=7
        )
        ax.set_xlabel("Trend increment", fontsize=11)
        ax.set_title(title, fontsize=12, fontweight="bold")
        ax.legend(fontsize=9)
    ...
    fig.suptitle(
        "Changepoint Estimation Bias (signed error at npost_max)\n"
        "Positive = estimated late, Negative = estimated early",
        fontsize=13,
        fontweight="bold",
    )
...
def plot_forecast_detection_summary(
    detection_results: dict, trend_increase, npre: int, title: str = "", savefile: str = None
):
    """
...
def plot_distance_time_series(
    sim_data, npre, dist_ts, stats=None, dist_measure="wasserstein", figsize=(12, 10), savefile=None
):
    """
    ...
    ax1.axvline(
        npre, color=CLR_TAU_TRUE, linestyle="--", linewidth=2, label=f"True changepoint (τ={npre})"
    )
    ...
    for color, tp in zip(colors, time_points):
        ...
        ax2.plot(
            x,
            gaussian_kde(data_ctr).evaluate(x),
            color=color,
            linestyle="--",
            linewidth=1.2,
            alpha=0.6,
        )
        ax2.plot(x, gaussian_kde(data_itv).evaluate(x), color=color, linewidth=1.8, label=f"t={tp}")

    ax2.set_title(
        "Distributions at Selected Time Points (Dashed = Control, Solid = Intervention)",
        fontsize=12,
        fontweight="bold",
    )
...
def plot_distribution_difference(
    sim_data, npre, dist_ts, stats=None, dist_measure="wasserstein", figsize=(12, 8), savefile=None
):
    """
    Two-panel figure for distribution change simulations.
    ...
    mean_diff = sim_data["sample_itv"][:, :nt].mean(axis=0) - sim_data["sample_ctr"][:, :nt].mean(
        axis=0
    )

    for ax in (ax1, ax2):
        ax.axvspan(1, npre, alpha=0.07, color="cornflowerblue")
        ax.axvline(npre, color=CLR_TAU_TRUE, linestyle="--", linewidth=2, label=f"True τ={npre}")
        if stats and stats.get("cpt"):
            ax.axvline(
                stats["cpt"],
                color=CLR_TAU_DET,
...
def plot_power_curves_mu_sigma(
    ...
    fig.suptitle(
        "Power Curves: Mean vs Variance Change Detection\n"
        "(detection rate vs post-intervention length, growing window)",
        fontsize=13,
        fontweight="bold",
    )
