...
if results_available:
        ...
        with delay_c2:
            if delay_change_type == "Mean (Mu)":
                delay_effect_pct = st.select_slider(
                    "Effect size",
                    options=list(mu_pct_to_trend.keys()),
                    value=30,
                ...
                delay_effect_pct = st.select_slider(
                    "Effect size",
                    options=list(sigma_pct_to_trend.keys()),
                    value=100,
                    format_func=lambda x: f"{x}%",
                    key="delay_effect_sigma",
                )
    ...
    with tab_null:
        ...
        cv_table = {
            ...
            "Null mean": [
                f"{np.mean(cv[f'npost_{label}']['null_dist']):.3f}"
                if f"npost_{label}" in cv
                else "—"
                for label, _ in null_keys
            ],
            "Null SD": [
                f"{np.std(cv[f'npost_{label}']['null_dist']):.3f}"
                if f"npost_{label}" in cv
                else "—"
                for label, _ in null_keys
            ],
        ...
        if fdr_dist_type == "Mean shifts (Mu)":
            ...
            st.plotly_chart(
                _fdr_fig_dist(
                    ...
                    EFFECT_SIZES_SIGMA_PCT,
                    "FDR heatmap — variance shifts (Sigma)",
                ),
                width="stretch",
            )
        else:
...
if results_available:
    ...
    with exp_c1:
        exp_type = st.radio(
            "Change type", ["Mean (Mu)", "Variance (Sigma)"], horizontal=True, key="exp_type"
        )
    with exp_c2:
        if exp_type == "Mean (Mu)":
            exp_pct = st.select_slider(
                "Effect size",
                options=list(mu_pct_to_trend.keys()),
                value=30,
                format_func=lambda x: f"{x}%",
    ...
    mean_diff = _sim_data["sample_itv"][:, :nt].mean(axis=0) - _sim_data["sample_ctr"][:, :nt].mean(
        axis=0
    )
    ...
    if _detected:
        ...
        _det_ann_side = (
            ("top left" if _true_ann_side == "top right" else "top right")
            if _close
            else _true_ann_side
        )
        for row in [1, 2]:
    ...
    fig_snap.add_trace(
        go.Scatter(
            x=_xr,
            y=gaussian_kde(_d_ctr).evaluate(_xr),
            mode="lines",
            name="control",
    ...
    fig_snap.add_trace(
        go.Scatter(
            x=_xr,
            y=gaussian_kde(_d_itv).evaluate(_xr),
            mode="lines",
            name="intervention",
...
with st.expander("⚙️ Simulation parameters", expanded=True):
    ...
    with col_c:
        s_mu_inc = st.slider(
            "Mean increment per month (Mu trend)", 0.0, 0.10, step=0.005, key="dist_p_mu_inc"
        )
        s_sigma_inc = st.slider(
            "SD increment per month (Sigma trend)", 0.0, 0.01, step=0.001, key="dist_p_sigma_inc"
        )
...
if s_run:
    ...
    st.session_state["dist_mini_history"].insert(
        0,
        {
            "label": _hist_lbl,
...
if "dist_mini_runs" in st.session_state and "all_sims" in st.session_state["dist_mini_runs"]:
    ...
    with sm1:
        st.metric(
            "Detection rate", f"{det_rate:.1%}", help=f"{n_detected} of {mr['n_sim']} simulations"
        )
